...
def demarrer_jeu():
    global en_jeu, score, obstacles, personnage, nombre_vies, delai_obstacle
    en_jeu = True
    score = 0
    nombre_vies = 3
    delai_obstacle = 5
...
# Police pour le score et les vies
...
while True:
    ...
    if en_jeu:
        ...
        if delai_obstacle > 0:
            ...
            for obstacle, _ in obstacles:
                ...
                if personnage_rect.colliderect(obstacle):
                    print("Collision !")
                    nombre_vies -= 1  # Réduire le nombre de vies
                    if nombre_vies > 0:
                        # Réinitialiser la position du personnage après une collision
                        personnage_rect.topleft = (50, hauteur_fenetre // 2 - 15)
                    else:
                        print("Game Over")
                        en_jeu = False
                        nombre_vies = 3  # Réinitialiser le nombre de vies
        ...
        texte_vies = police.render(f"Vies: {nombre_vies}", True, rouge)
        fenetre.blit(texte_vies, (largeur_fenetre - 120, 10))
